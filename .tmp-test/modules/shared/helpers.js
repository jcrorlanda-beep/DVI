...
export function getPermissionsForRole(role, defs) {
    const def = defs.find((d) => d.role === role);
    return def ? def.permissions : [];
}
export function hasPermission(role, defs, permission) {
    const perms = getPermissionsForRole(role, defs);
    return perms.includes(permission);
}
export function getWorkLogMinutes(log) {
    if (log.endedAt)
...
export function getResponsiveSpan(span, isCompactLayout) {
    return isCompactLayout ? "span 12" : `span ${span}`;
}
export function canReleaseRO(ro) {
    return ro.status === "Ready Release";
}
export function canExecuteWorkLine(ro, line) {
...
export function isWorkLineBlockedByParts(line, partsRequests) {
...
export function canCompleteWorkLine(ro, line, partsRequests) {
    if (ro.status !== "In Progress" && ro.status !== "Waiting Parts")
        return false;
...
export function canMoveToQualityCheck(ro) {
    if (ro.status !== "In Progress" && ro.status !== "Waiting Parts")
        return false;
...
export function canTransitionROStatus(ro, to) {
...
// --- REPORT HELPERS ---
export function getTechnicianProductivity(ros, workLogs, users) {
    const map = new Map();
    for (const ro of ros) {
...
export function getAdvisorSalesProduced(ros, invoices) {
    const map = new Map();
    for (const ro of ros) {
...
export function getRepeatCustomerFrequency(ros) {
    const map = new Map();
    for (const ro of ros) {
...
export function getQcPassFailSummary(qcRecords) {
...
export function getWaitingPartsAging(ros, partsRequests) {
    const now = Date.now();
    const result = [];
    for (const ro of ros) {
...
export function getBackjobRate(ros, backjobs) {
