...
function IntakePage({
  ...
  const [form, setForm] = useState<IntakeForm>(() => {
    ...
    const savedDraft = readLocalStorage<IntakeForm | null>(STORAGE_KEYS.intakeDraft, null);
    return savedDraft ? { ...defaultForm, ...savedDraft, assignedAdvisor: savedDraft.assignedAdvisor || currentUser.fullName } : defaultForm;
  });
  const [search, setSearch] = useState("");
  const [error, setError] = useState("");
  const [showDraftRestore, setShowDraftRestore] = useState(() => hasNonEmptyValues(readLocalStorage<IntakeForm | null>(STORAGE_KEYS.intakeDraft, null)));
  const intakeDraft = useDraftAutosave(STORAGE_KEYS.intakeDraft, form, true);
...
function BookingsPage({
  ...
  const bookingDraftInitial = readLocalStorage<BookingForm | null>(STORAGE_KEYS.bookingDraft, null);
  ...
  const bookingDraft = useDraftAutosave(STORAGE_KEYS.bookingDraft, form, hasNonEmptyValues(form));
